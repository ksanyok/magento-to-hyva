...
define([
    ...
    $.widget('mage.amnotification', {
        ...
        _reloadDefaultContent: function () {
            ...
            if (this.priceAlert && this.priceAlert.length) {
        ...
        showStockAlert: function (code) {
            ...
            var insertwrapper = $('.towishlist')[0];
            ...
            }).html(code).insertBefore(insertwrapper),
        ...
        flagOutOfStock: function () {
            var selected = this._getSelectedKey();
            var selectList = selected.split(',');

            for (const [key, element] of Object.entries(this.options.xnotif)) {
                if(key.match(selectList[0])) {
                    var size = key.split(',')[1];
                    if(element.is_in_stock) {
                        $('#product-options-wrapper').find('div[option-id="'+size+'"]').removeClass('out-of-stock').removeClass('disabled');
                    } else {
                        $('#product-options-wrapper').find('div[option-id="'+size+'"]').addClass('out-of-stock').addClass('disabled');
                    }
                }
            }
        ...
        _getSelectedKey: function () {
            var selectedKey = [];
            this.settingsForKey
                = $('#product-options-wrapper').find('select.super-attribute-select, div.swatch-option.selected:not(.slick-cloned), select.swatch-select');
